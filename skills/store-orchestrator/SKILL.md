...
- Separate desired state from observed state. Keep remote IDs and observations in `store.state.json` (a non-secret cache), evidence/questions in `store-plan.md`, and readiness in `release-readiness.md`, unless the repository has equivalent conventions. Cache contents are never proof of current remote state. Follow [release-docs](references/release-docs.md) for the shape of these files.
...
- Compare the repository checklist AND [release checklist](references/release-checklist.md), preserving project requirements. If no checklist exists, use the bundled baseline. Record applicability, evidence, status, owner, and next action for every applicable item. Clearly state whether screenshot files were merely found, technically validated, visually inspected, user approved, uploaded, or confirmed processed. This skill never captures screenshots; when the `store-screenshots` skill is installed, name it as the capture route, otherwise leave capture pending with a precise capture list.
...
- When the app mirrors entitlements in its own backend, run the subscription lifecycle-event check from the release checklist and report each unhandled event type as blocked.
